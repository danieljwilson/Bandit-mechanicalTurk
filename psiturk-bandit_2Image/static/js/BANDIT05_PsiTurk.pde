       ...
       text("Press the 'B' key to continue", width/2-135, 38);
        ...
        if(key == 'B' || key == 'b'){
          ...
          if (trial == 10 || leftSequentialSelect == 5 || rightSequentialSelect == 5){
            ...
            //INSERT VARIABLES TO RECORD HERE
            /////////////////////////////////
            if(javascript!=null){
              javascript.recordData(trialTotal, round, winPercentageLeft, winPercentageRight, trial, selected, win, wins, losses, money, leftSequentialSelect, rightSequentialSelect);
            }
...
void printOutcome(){
  ...
  if(textNum ==2){
    text("LOSE", textPos+8, height/2+13);
    }
}
